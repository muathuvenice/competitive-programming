...
void process(){
    ...
    FOR(i, 1, n){
        ...
        c.emplace_back(p[i].first.second);
    }
    sort(all(c));
    uni(c);
//    for(int y : c)cout << y << ' ';cout << '\n';
    sort(p + 1, p + 1 + n);
    int mx = c.size();

    int k = 1;
    Interval it = Interval(mx);
    long long answer = 0;
    for(int i = 1; i <= n; ){

        int j = i;
        while(j <= n && p[j].first.first == p[i].first.first){
            int l = lower_bound(all(c), p[j].first.second - h + 1) - c.begin() + 1;
            int r = lower_bound(all(c), p[j].first.second) - c.begin() + 1;
            it.update(l, r, p[j].second);
            ++j;
        }
        while(p[i].first.first - p[k].first.first + 1 > w){
            int l = lower_bound(all(c), p[k].first.second - h + 1) - c.begin() + 1;
            int r = lower_bound(all(c), p[k].first.second) - c.begin() + 1;
            it.update(l, r, -p[k].second);
            ++k;
        }
        maximizell(answer, it.it[1]);
        i = j;
    }
    cout << answer;
}
